...
- Implemented **Pre-Computed Talent Profile Evaluation** on branch `claude/talent-profile-precomputed-scoring`. Full plan → [`docs/features/talent-profile-precomputed-scoring.md`](../features/talent-profile-precomputed-scoring.md).
- Problem: `/recruiter/dashboard/find-talents` re-scored every candidate with a fresh LLM call on every single search (up to 30 calls/search, raw bio re-sent each time, CV never even read). Fix: evaluate a talent's profile once at admin approval — now including extracted CV text — cache it, and reuse it on every subsequent search.
- New `app/lib/ai/evaluate-talent-profile.ts` — `evaluateAndStoreTalentProfile(userId)`: reads skills/bio/resume_* fields, extracts CV text via `cv_url` (new `app/lib/pdf/extract-pdf-text.ts`, refactored out of `pdf-to-profile/route.ts` so both share one extraction path), calls Groq once, stores a structured `TalentProfileSummary` (seniority, years, top skills, strengths, domains, notable projects, summary) plus `cv_text` on `goodhive.talents`. Never throws — a failure is logged and self-heals later.
- Trigger points: `app/api/admin/talents/status/route.ts` (single approval) and `app/api/admin/talents/bulk-approve/route.ts` (bulk, 3-at-a-time concurrency matching the existing top-talents batching) — only when the `talent` role is being approved. Awaited inline (same pattern as the existing approval email send) so it actually completes before the serverless invocation ends, but a failure never fails the approval itself.
- `app/api/recruiter/top-talents/route.ts` now SELECTs the cached `ai_profile_summary`/`ai_profile_summary_version`/`ai_profile_stale` and feeds the compact cached summary into `computeMatchScore` instead of raw bio text. Self-healing fallback: a candidate with no valid cache gets evaluated inline for this one search (so the *next* search is fast for them too) and falls back to raw-bio scoring for the current search regardless of outcome — no candidate is ever silently dropped.
- `app/lib/ai/match-score.ts` gained an optional `talentProfileSummary` param — when absent (company-side `top-candidates` is untouched), behavior is byte-for-byte identical to before.
- Staleness: `app/api/talents/my-profile/route.ts` now flags `ai_profile_stale = true` when an already-approved talent's save touches skills/description/about_work/resume_*/cv_url — cheap flag flip, no AI call on the save path, defensively gated behind an `information_schema` column-existence check (same pattern this file already uses for `RESUME_IMPORT_COLUMNS`) so a deploy landing ahead of the migration doesn't break saves.
- New `app/api/admin/talents/evaluate-pending/route.ts` (admin-only) and `scripts/backfill-talent-profile-evaluations.ts` (`pnpm backfill:talent-profile-evaluations`) both evaluate approved talents with a missing/stale/outdated-version cache — the backfill script covers the current already-approved pool once the migration lands.
- Migration: `app/db/migrations/add_talent_profile_evaluation.sql` (mirrored to `database/migrations/`) adds `cv_text`, `ai_profile_summary` (jsonb), `ai_profile_summary_version`, `ai_profile_evaluated_at`, `ai_profile_stale` to `goodhive.talents`, plus a partial index for the "needs evaluation" query. **Not yet applied to any environment.**
- Verified: `pnpm tsc --noEmit` clean, `pnpm lint` clean (only pre-existing warnings elsewhere, none in touched files). Smoke-tested all five touched/new API routes against a real dev server (`next dev -p 3001`) — each returns its expected auth/validation response (401/400) with no import or runtime errors in server logs.
- **Migration applied to dev** (2026-08-28, explicit authorization). Verified via `\d goodhive.talents`: all 5 new columns present, partial index created.
- **Backfill run against dev — 47/47 approved talents covered.** Along the way, found and fixed a real, pre-existing, app-wide bug unrelated to this feature: `lib/ai/groq.ts`'s `DEFAULT_MODEL_POOL` and `lib/gemini.ts`'s Groq shim both hardcoded 4 Groq model IDs that are now 404/decommissioned — meaning *every* Groq-backed AI feature in the app (match-score, pdf-to-profile, outreach-email, superbot, hiring-coach, etc.) was silently failing 100% of the time in this environment before this fix. Replaced with the 4 models this account's key actually has access to (`qwen/qwen3.8-27b`, `qwen/qwen3.6-27b`, `openai/gpt-oss-120b`, `openai/gpt-oss-20b`), verified live against `GET /openai/v1/models`. Also updated the cosmetic model-name/color mapping in `app/components/admin/GroqUsageSection.tsx` to match. **This fix is unrelated to precomputed scoring and should probably be called out as its own fix in any PR/changelog.**
- Backfill also surfaced two robustness bugs in the new evaluation code itself, both fixed: (1) reasoning-capable pool models (qwen3) emit a hidden `<think>` block that consumed the whole token budget before ever reaching JSON — fixed with an explicit "no thinking, JSON only" prompt instruction (`/no_think`, Qwen3's documented directive) and a bump from `maxTokens: 900` to `2500`; (2) a Postgres "invalid byte sequence" error (code 22021) from NUL bytes that `pdf-parse` occasionally extracts from PDFs, which Postgres text/JSONB columns reject outright — fixed with a `sanitizeForStorage` pass on both `cv_text` and the bio before they're used or stored.
- End-to-end verified via direct DB queries (read-only) and a standalone script exercising the real code paths (not just types): sampled two cached evaluations and confirmed the output is genuinely CV-grounded (specific project names, accurate years-of-experience, real tech stacks — not generic filler); then called `computeMatchScore` with a real cached summary the way `top-talents/route.ts` does, confirmed a sharp, evidence-based score, and confirmed `goodhive.groq_usage` shows **zero** new `feature = 'evaluate-talent-profile'` rows from that search call — proving the cache is actually being consumed instead of re-evaluated.
- Not yet done: a real admin-UI approval click-through (blocked on having real admin credentials — used direct backfill script instead, which exercises the identical `evaluateAndStoreTalentProfile` code path the approval routes call), and applying the migration + the `lib/ai/groq.ts`/`lib/gemini.ts` model-pool fix to prod. **No commit made yet** — working tree has the implementation uncommitted on the feature branch pending review.

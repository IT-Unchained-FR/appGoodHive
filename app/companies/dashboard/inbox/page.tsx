...
type ConversationFilter = "all" | "unread" | "awaiting_company";
...
export default function CompanyInboxPage() {
  ...
  const loadThreads = useCallback(
    ...
    try {
      ...
      const response = await fetch(`/api/conversations/company?${params.toString()}`);
      if (!response.ok) {
      ...
      if (notifyOnError) {
        toast.error("Failed to load applicant conversations");
      }
    } finally {
      if (showLoader) {
  ...
  const talentSummary = buildConversationSummary(
    threadDetail?.thread.talent_headline,
    "Applicant profile summary will appear here once more profile details are available.",
  );
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(251,191,36,0.12),_transparent_24%),linear-gradient(180deg,_#fffdf7_0%,_#f8fafc_28%,_#f8fafc_100%)] px-4 pb-10 pt-6 sm:px-6 lg:px-8 xl:px-10">
      <div className="mx-auto flex w-full max-w-[1560px] flex-col gap-6">
        <div className="overflow-hidden rounded-[32px] border border-slate-200/70 bg-[linear-gradient(135deg,_#1f2937_0%,_#111827_58%,_#0f172a_100%)] text-white shadow-[0_24px_80px_rgba(15,23,42,0.22)]">
          <div className="flex flex-col gap-8 px-6 py-7 sm:px-8 lg:flex-row lg:items-end lg:justify-between lg:px-10">
            <div className="max-w-3xl">
              <div className="mb-4 flex flex-wrap items-center gap-3">
                <span className="inline-flex items-center rounded-full border border-white/15 bg-white/10 px-4 py-1.5 text-[11px] font-semibold uppercase tracking-[0.24em] text-amber-200">
                  Applicant Inbox
                </span>
                <span className="inline-flex items-center rounded-full border border-emerald-400/20 bg-emerald-400/10 px-3 py-1 text-xs font-medium text-emerald-100">
                  Live sync enabled
                </span>
              </div>
              <h1 className="max-w-2xl text-3xl font-semibold tracking-tight text-white sm:text-4xl">
                Keep every applicant conversation moving from one focused workspace
              </h1>
              <p className="mt-4 max-w-2xl text-sm leading-7 text-slate-300 sm:text-base">
                Review the latest replies, see who is active, and answer inside the context
                of the job instead of jumping between email threads and profile tabs.
              </p>
            </div>

            <div className="grid gap-3 sm:grid-cols-3 lg:min-w-[520px]">
              <MetricCard label="Conversations" value={threads.length} accent="amber" />
              <MetricCard label="Unread" value={totalUnread} accent="emerald" />
              <MetricCard
                label="Focus"
                value={selectedThreadId ? "1 live" : "Idle"}
                accent="sky"
              />
            </div>
          </div>
            ...
            <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
              <div className="max-w-2xl">
                ...
                <p className="mt-2 text-sm leading-6 text-slate-600">
                  Search threads, watch unread movement, and reply without losing the hiring
                  context around each application.
                </p>
              </div>
              ...
              <div className="flex flex-col gap-3 lg:min-w-[430px] lg:max-w-[520px] lg:flex-row">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    placeholder="Search by applicant, job, or message"
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    ...
                    { label: "Awaiting you", value: "awaiting_company" },
                  ].map((option) => (
                    <button
                      key={option.value}
                ...
                {isLoadingThreads ? (
                  ...
                  <div className="flex h-full min-h-[420px] flex-col items-center justify-center rounded-[28px] border border-dashed border-slate-200 bg-white px-6 text-center shadow-sm">
                    ...
                    <p className="mt-3 max-w-xs text-sm leading-6 text-slate-600">
                      New applications will appear here automatically and open as private
                      message threads.
                    </p>
                  </div>
                ) : (
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-start gap-3">
                            <Avatar
                              name={thread.talent_name}
                              imageUrl={thread.talent_image_url}
                            />
              ...
              {!selectedThreadId || (!threadDetail && isLoadingThread) ? (
                ...
                <div className="flex h-full min-h-[740px] flex-col items-center justify-center px-6 text-center">
                  ...
                  <p className="mt-3 max-w-md text-sm leading-6 text-slate-600">
                    Open any applicant thread to review the latest messages and move the
                  ...
                  <div className="border-b border-slate-200 bg-[linear-gradient(180deg,_rgba(255,251,235,0.65)_0%,_#ffffff_80%)] px-4 py-5 sm:px-6">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex min-w-0 items-start gap-3 sm:gap-4">
                        ...
                        <Avatar
                          name={threadDetail.thread.talent_name}
                          imageUrl={threadDetail.thread.talent_image_url}
                          large
                        />
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            <h3 className="truncate text-xl font-semibold text-slate-950">
                              {threadDetail.thread.talent_name}
                            </h3>
                            <ApplicationStatusBadge
                          ...
                          <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-slate-500">
                            ...
                            <span className="inline-flex items-center gap-1.5">
                              ...
                              {threadDetail.thread.talent_email}
                            </span>
                          </div>
                      ...
                      <Link
                        href={`/talents/${threadDetail.thread.talent_user_id}`}
                        target="_blank"
                      ...
                      <div className="overflow-hidden rounded-[28px] border border-slate-200 bg-[linear-gradient(180deg,_#ffffff_0%,_#f8fafc_100%)] shadow-sm">
                        ...
                        <div className="flex flex-col gap-3 border-t border-slate-200/80 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-5">
                          <p className="max-w-xl text-xs leading-5 text-slate-500">
                            Keep replies specific to scope, timing, and next steps so applicants
...
function MetricCard({
  ...
  const accentStyles = {
    ...
    emerald: "from-emerald-400/20 to-emerald-300/10 text-emerald-100",
    sky: "from-sky-400/20 to-sky-300/10 text-sky-100",
  };

  return (
    <div className={`rounded-[24px] border border-white/10 bg-gradient-to-br ${accentStyles[accent]} p-5 shadow-inner`}>
...
function ThreadStatusBadge({
  ...
  const labelMap: Record<string, string> = {
    open: "Open",
    awaiting_company: "Awaiting you",
    awaiting_talent: "Awaiting talent",
    archived: "Archived",
    closed: "Closed",
  ...
  const colorMap: Record<string, string> = {
    ...
    awaiting_company: "bg-amber-100 text-amber-800",
    awaiting_talent: "bg-sky-100 text-sky-800",
